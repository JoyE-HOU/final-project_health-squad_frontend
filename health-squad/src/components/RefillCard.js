...
const RefillCard = ({med}) => {
    ...
    //add days to med.medication.count
    const date = new Date();
    const sum1 = addDays(date, med.medication.count);
    // console.log(sum1);
    const sub1 = subDays(sum1, 14);
    // console.log(sub1);
    const refillDate = format(new Date(sub1), 'MM/dd/yyyy');
    ...
    return(
        <div className="card">
          <div className="card-body">
            ...
            <h6 className="card-subtitle mb-2 text-muted">Remember to order refill around:</h6>
            <p className="card-text">{refillDate}</p>
